...
// 직군별 한 줄 요약(결론) — 카드 최상단 헤드라인으로 사용
const SUMMARY = {
  designer: "사용자 흐름과 일관성에 닿는 변경이에요 — 동선·패턴·인터랙션 의도 순으로 짚었어요.",
  frontend: "화면단에서 소화할 수 있는 변경으로 보여요 — 구현 방식·레이아웃·반응형만 확인하면 돼요.",
  backend: "서버·API에 미치는 영향은 크지 않아 보여요 — 데이터와 연동 범위만 확인하면 돼요.",
  pm: "일정 부담이 크지 않은 변경이에요 — UX 개선 효과와 릴리즈 리스크를 함께 봤어요.",
  qa: "회귀 위험을 먼저 짚어야 하는 변경이에요 — 테스트 케이스와 재현 경로를 잡아뒀어요.",
  marketer: "사용자 가치로 풀어 말할 수 있는 변경이에요 — 메시징과 출시 타이밍 관점으로 봤어요.",
};
...
async function translateMock(text, subject, target) {
  ...
  const isDefault = initial && initial.concerns === target.concerns;
  const items = target.concerns.split(/[,，]/).map((s) => s.trim()).filter(Boolean).slice(0, 5);
  const headline = isDefault && SUMMARY[target.id]
    ? SUMMARY[target.id]
    : `${items[0]} 관점을 중심으로 짚었어요${items.length > 1 ? ` — ${items.slice(1, 3).join("·")}도 함께 확인하면 좋아요.` : "."}`;
  let points = isDefault ? DEMO[target.id] : null;
  if (!points) points = items.map((l) => ({ label: l, detail: `${l} 관점에서 이 작업을 검토할 때 먼저 확인할 부분입니다.` }));
...
function LensLogo({ size = 36 }) {
  ...
  return (
    <svg className="lens-logo" width={size} height={size} viewBox="0 0 84 84" fill="none" role="img" aria-label="프리즘">
        ...
        <div className="main-inner">
              ...
              {editingWork ? (
                <div className="work-edit">
                  ...
                  <div className="work-edit-foot">
                    <span className="work-edit-hint">저장하면 열린 직군 패널이 새 작업 기준으로 다시 생성됩니다.</span>
          ...
          {concernsDirty && openIds.length > 0 && (
            <div className="notice">
              ...
              <span style={{ flex: 1 }}>관심사가 변경되었어요. 다시 시도하면 열린 직군 패널에 새 관심사가 반영됩니다.</span>
          ...
          {flow && !flow.done ? (
            ...
            <div style={{ textAlign: "center", padding: "48px 24px", color: "var(--dim)", fontSize: 14 }}>
              왼쪽 목록에서 <span style={{ color: "var(--mfg)" }}>직군</span>을 누르면, 이 작업이 그 직군의 언어로 변환되어 여기에 나란히 쌓입니다.
            </div>
          )}
          ...
          <div className="ph-wrap">
            ...
            {!dockHasText && (
              <span className={"ph-shimmer" + (listening ? " coral" : "")} style={{ fontSize: 15, left: 4 }}>
                {listening ? "듣고 있어요… 말씀해 보세요" : "작업을 새로 입력하거나 수정하세요 — 열린 직군 패널이 한 번에 갱신됩니다"}
              </span>
            )}
          </div>
